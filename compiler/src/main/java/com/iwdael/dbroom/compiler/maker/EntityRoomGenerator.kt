...
class EntityRoomGenerator(private val clazz: Class) : Generator {
    ...
    private fun replacer() = MethodSpec.methodBuilder("replace")
        .addModifiers(Modifier.PUBLIC)
        .addParameter(clazz.sqlReplacerClassName(), "replacer")
        .addStatement("List<\$T> finds = find(new SimpleSQLiteQuery(replacer.findSelection, replacer.findBindArgs))", clazz.asTypeName())
        .beginControlFlow("if (finds.isEmpty())")
        .addStatement("return insert(new SimpleSQLiteQuery(replacer.insertSelection, replacer.insertBindArgs))")
        .nextControlFlow("else")
        .addStatement("return update(new SimpleSQLiteQuery(replacer.updateSelection, replacer.updateBindArgs))")
        .endControlFlow()
        ...
        MethodSpec.methodBuilder("renewal${field.name.charUpper()}")
            ...
            .addAnnotation(
                AnnotationSpec.builder(Query::class.java)
                    .addMember(
                        "value",
                        "\"INSERT OR REPLACE INTO ${clazz.roomTableName()} (${clazz.primaryKey().colName()} , ${field.colName()}) VALUES (:${clazz.primaryKey().name} , :${field.name})\""
                    )
                    .build()
            )
            .addParameter(
