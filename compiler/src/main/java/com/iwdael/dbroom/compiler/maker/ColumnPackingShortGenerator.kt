 ...
 */
class ColumnPackingShortGenerator : Generator {
    override fun classFull() = "${packageName()}.${simpleClassName()}"
    override fun simpleClassName() = PACKING_SHORT_COLUMN.simpleName()
    override fun packageName() = PACKING_SHORT_COLUMN.packageName()

    override fun generate(filer: Filer) {
        JavaFile
            .builder(
                packageName(), TypeSpec.classBuilder(simpleClassName())
                    .superclass(ParameterizedTypeName.get(PACKING_COLUMN, SHORT_PACKING))
