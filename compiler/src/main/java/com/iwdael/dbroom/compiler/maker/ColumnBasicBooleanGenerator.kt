 ...
 */
class ColumnBasicBooleanGenerator : Generator {
    override fun classFull() = "${packageName()}.${simpleClassName()}"
    override fun simpleClassName() = BASIC_BOOLEAN_COLUMN.simpleName()
    override fun packageName() = BASIC_BOOLEAN_COLUMN.packageName()

    override fun generate(filer: Filer) {
        JavaFile
            .builder(
                packageName(), TypeSpec.classBuilder(simpleClassName())
                    .superclass(ParameterizedTypeName.get(BASIC_COLUMN, BOOLEAN_PACKING))
