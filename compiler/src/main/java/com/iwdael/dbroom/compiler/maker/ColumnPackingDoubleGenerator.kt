 ...
 */
class ColumnPackingDoubleGenerator : Generator {
    override fun classFull() = "${packageName()}.${simpleClassName()}"
    override fun simpleClassName() = PACKING_DOUBLE_COLUMN.simpleName()
    override fun packageName() = PACKING_DOUBLE_COLUMN.packageName()

    override fun generate(filer: Filer) {
        JavaFile
            .builder(
                packageName(), TypeSpec.classBuilder(simpleClassName())
                    .superclass(ParameterizedTypeName.get(PACKING_COLUMN, DOUBLE_PACKING))
