    ...
    override fun process(annos: MutableSet<out TypeElement>, env: RoundEnvironment): Boolean {
        ...
        (env.getElementsAnnotatedWith(Entity::class.java) ?: arrayListOf())
            ...
            .apply {
                ...
                ConditionPackingFloatGenerator().generate(processingEnv.filer)
                ConditionBasicFloatGenerator().generate(processingEnv.filer)
                ConditionPackingShortGenerator().generate(processingEnv.filer)
                ConditionBasicShortGenerator().generate(processingEnv.filer)
                ConditionPackingCharGenerator().generate(processingEnv.filer)
                ConditionBasicCharGenerator().generate(processingEnv.filer)
                ConditionPackingDoubleGenerator().generate(processingEnv.filer)
                ConditionBasicDoubleGenerator().generate(processingEnv.filer)
                ConditionPackingLongGenerator().generate(processingEnv.filer)
                ConditionBasicLongGenerator().generate(processingEnv.filer)
                ConditionPackingByteGenerator().generate(processingEnv.filer)
                ConditionBasicByteGenerator().generate(processingEnv.filer)
                ConditionPackingBooleanGenerator().generate(processingEnv.filer)
                ConditionBasicBooleanGenerator().generate(processingEnv.filer)
