...
class EntityNotifierGenerator(private val clazz: Class) : Generator {
    ...
    override fun generate(filer: Filer) {
        if (!clazz.useNotifier() and !clazz.useRoom())
            JavaFile
                .builder(
                    packageNameGen, TypeSpec
                        .classBuilder(simpleClassNameGen)
    ...
    private fun renewal() = MethodSpec.methodBuilder("renewal")
        .addModifiers(Modifier.PUBLIC, Modifier.STATIC)
        .addParameter(clazz.asTypeName(), clazz.classSimpleName.charLower())
        .returns(
            ParameterizedTypeName.get(
                ClassName.get(Pair::class.java),
                ClassName.get(String::class.java), ArrayTypeName.of(Object::class.java)
            )
        )
        .addStatement("Pair<String, Object[]> pair = new Pair<>(null, null)")
        .addStatement("\$T<Object> values = new \$T<>()", ClassName.get(List::class.java), ClassName.get(ArrayList::class.java))
        .addStatement("\$T builder = new \$T()", ClassName.get(StringBuilder::class.java), ClassName.get(StringBuilder::class.java))
        .addStatement("int propertyCount = 0")
        .addStatement("builder.append(\"INSERT OR REPLACE INTO \$N (\")", clazz.roomTableName())
        .apply {
            clazz.roomFields().forEach {
                if (!it.asTypeName().isPrimitive) beginControlFlow("if (${clazz.classSimpleName.charLower()}.${it.getter.name}() != null)")
                addStatement("builder.append(propertyCount > 0 ? \",\" : \"\")")
                addStatement("builder.append(\"${it.colName()}\")")
                addStatement("values.add(${clazz.classSimpleName.charLower()}.${it.getter.name}())")
                addStatement("propertyCount++")
                if (!it.asTypeName().isPrimitive) endControlFlow()
            }
        }
        .addStatement("builder.append(\") VALUES (\")")
        .beginControlFlow("for (int index = 0; index < propertyCount; index++)")
        .addStatement("builder.append(index > 0 ? \",\" : \"\")")
        .addStatement("builder.append(\"?\")")
        .endControlFlow()
        .addStatement("builder.append(\")\")")
        .addStatement("return new Pair<>(builder.toString(), values.toArray())")
        .build()
}