...
class DbRoomProcessor(private val env: SymbolProcessorEnvironment) : SymbolProcessor {
    override fun process(resolver: Resolver): List<KSAnnotated> {
        ...
        val dbRoomCreators = resolver.getSymbolsWithAnnotation(DbRoomCreator::class.java.name).toList()
        ...
        resolver.getSymbolsWithAnnotation(Entity::class.java.name)
            ...
            .map { it.asKspClass }
            .apply {
                val dao = resolver.getSymbolsWithAnnotation(Dao::class.java.name).map { it.asKspClass }.toList()
